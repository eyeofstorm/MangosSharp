...
namespace Mangos.Cluster.Handlers
{
    public class WC_Handlers_Social
    {
        ...
        public void On_CMSG_WHO(PacketClass packet, ClientClass client)
        {
            ...
            var namePlayer = clusterServiceLocator._Functions.EscapeString(packet.GetString());
            var nameGuild = clusterServiceLocator._Functions.EscapeString(packet.GetString());
        ...
        public void On_CMSG_ADD_FRIEND(PacketClass packet, ClientClass client)
        {
            ...
            if (q.Rows.Count > 0)
            {
                ...
                var FriendSide = clusterServiceLocator._Functions.GetCharacterSide(q.Rows[0].As<byte>("char_race"));
                q.Clear();
