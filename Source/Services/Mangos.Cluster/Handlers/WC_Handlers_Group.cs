    ...
    public class WC_Handlers_Group
    {
            ...
            public void SendGroupList()
            {
                ...
                for (byte i = 0, loopTo = (byte)(Members.Length - 1); i <= loopTo; i++)
                {
                    if (Members[i] is object)
                    {
                        ...
                        var MemberFlags = (byte)(i / clusterServiceLocator._Global_Constants.GROUP_SUBGROUPSIZE);
        ...
        public void On_CMSG_GROUP_INVITE(PacketClass packet, ClientClass client)
        {
            if (packet.Data.Length - 1 < 6)
                return;
            packet.GetInt16();
            var Name = clusterServiceLocator._Functions.CapitalizeName(packet.GetString());
